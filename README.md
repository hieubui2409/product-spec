...
> 🎮 **See the whole rig in action — [live showcase: `hieubui2409.github.io/product-spec`](https://hieubui2409.github.io/product-spec/):** a standalone,
> interactive tour of every feature of all four skills (dark, animated, bilingual EN/VI). Prefer local?
> Open [`showcase/index.html`](showcase/index.html) in any browser — no build step, no network calls.
...
| **Hooks** | `.claude/hooks/` | The `product-spec-critique` post-validate re-critique nudge, the `product-spec` memory-gap enforcement hook, and telemetry's **5 sink hooks** (skill invocation, script duration, sub-agent outcome, bash timing, session summary) — auto-registered at install. |
...
**Copyright (c) 2026 Hieu Bui.** Licensed under the **GNU Affero General Public License v3.0 (AGPL-3.0)** — see
[`LICENSE`](LICENSE).

In short: you may use, study, modify, and share this freely, **but if you run a modified version as a network service or
distribute it, you must release your complete source under AGPL-3.0 too.** You may **not** fold this code into a
closed-source / proprietary commercial product — that requires a **separate commercial license** (contact the copyright
holder). See [`CONTRIBUTING.md`](CONTRIBUTING.md) for how contributions are licensed (inbound = outbound).
...
| **Hook** | `.claude/hooks/` | Hook nhắc re-critique sau validate của `product-spec-critique`, hook memory-gap (enforcement) của `product-spec`, và **5 sink hook** của telemetry (skill invocation, script duration, sub-agent outcome, bash timing, session summary) — tự đăng ký khi cài. |
...
> 🎮 **Xem cả bộ chạy thực tế — [bản online: `hieubui2409.github.io/product-spec`](https://hieubui2409.github.io/product-spec/):** một bản tham quan
> tương tác, tự chứa về **toàn bộ tính năng** của cả bốn skill (nền tối, có hoạt ảnh, song ngữ EN/VI).
> Muốn chạy cục bộ? Mở [`showcase/index.html`](showcase/index.html) bằng bất kỳ trình duyệt nào — không cần build, không gọi mạng.
...
**Copyright (c) 2026 Hieu Bui.** Cấp phép theo **GNU Affero General Public License v3.0 (AGPL-3.0)** — xem [`LICENSE`](LICENSE).

Tóm tắt: bạn được tự do dùng, nghiên cứu, sửa và chia sẻ, **nhưng nếu chạy bản đã sửa dưới dạng dịch vụ qua mạng hoặc phân phối nó, bạn phải công bố toàn bộ source dưới AGPL-3.0**. Bạn **không được** đưa code này vào sản phẩm thương mại đóng/độc quyền — việc đó cần một **license thương mại riêng** (liên hệ chủ bản quyền). Xem [`CONTRIBUTING.md`](CONTRIBUTING.md) về cách đóng góp được cấp phép (inbound = outbound).
