...
### P10a · #2 spec-validate.yml CI — CVR-F07/POX-F01 (Q4=a) · 2026-06-12 (build-new)
- Template GH Action ship qua `render_embedded` ở **bundle-root** (loại khỏi generic walk cả 2 installer), installer
  **hỏi PO** opt-in (`NON_INTERACTIVE`-aware, `INSTALL_SPEC_VALIDATE=1`/`-InstallSpecValidate` force, giữ file cũ trừ
  `FORCE_OVERWRITE`) → `.github/workflows/`. Chạy 3 check `--root .` trên `docs/product/`, summary VI vào
  `$GITHUB_STEP_SUMMARY`, graceful skip khi thiếu docs/product. Tránh `${{ }}` (đụng `{{TOKEN}}` subst). Khuyên xoá
  `python-package.yml`. INSTALL.md EN/VI.
- **Critique-challenge (REAL risk):** "pip pyyaml đủ cho CI?" → audit transitive import 11 module của 3 check →
  **chỉ `yaml` third-party** (còn lại stdlib+sibling) → đủ, không kéo google-genai. RESOLVED. Golden synthetic cập nhật
  (thêm `spec-validate.yml` — embed mới có chủ đích).
...
### P8 (#15) · monthly change-log rotation writer + cross-file audit-trail read + orphan check (build-new) · 2026-06-13 (build-new)

- [x] monthly rotation writer — new `change_log_writer.py` sibling: `write_change_log_entry(root, entry_md, *, when=None)` appends to `docs/product/change-log/<YYYY-MM>.md` (auto-creates dir; dedup guard on date+artifact+action triple; injectable `when` for deterministic tests). `generate_templates.py --type change_log_entry --write` now actually persists via the writer (previously a no-op for this type).
- [x] 4 ref-doc instruction rewires — `references/workflow-validate.md` (~L266 + L320), `references/workflow-interview.md` (~L18 + L301), `references/workflow-update.md` (~L63): each change-log-append instruction now invokes `write_change_log_entry(root, rendered_entry_md)` explicitly. Surrounding prose byte-intact; grep-asserted by `test_ref_docs_invoke_writer`.
- [x] cross-file audit-trail read — `assemble_audit_trail._parse_change_log` refactored: new `_change_log_paths(root)` returns `[legacy change-log.md] + sorted(change-log/*.md)`; `_parse_one_change_log_file` extracts entries from a single text; merge deduplicates on (date, first-artifact, action), sorts chronologically. Legacy-only back-compat preserved (test 5 green).
- [x] orphan-path check — `_live_artifact_ids(root)` returns `Optional[set]` (None = graph unavailable, empty set = no artifacts); change-log events whose artifact ID is absent from the live graph are flagged `reconciled=False`, folded into the existing `unreconciled_count` mechanism. No new schema keys.
- 8 tests: test_writer_appends_to_month_file, test_writer_rotates_across_months, test_writer_dedup_idempotent (negative), test_assemble_reads_legacy_and_rolled, test_legacy_only_still_read (negative/back-compat), test_missing_artifact_path_flagged, test_existing_path_not_flagged (negative), test_ref_docs_invoke_writer. All 8 GREEN. P12 ascii-truncation tests preserved (3 green). Suite 762 passed / 1 pre-existing dogfood-state failure. CONTRIBUTING:69 gate 237 passed. Context footprint baseline regenerated (+180 tokens product-spec, ref-doc growth). DEC-6 recorded. → EVIDENCE P8-15.
