...
### FIX-WAVE (post code-review) · 3 commits `b603637` / `2168ea8` / `f519009`
Adversarial 5-lens review (correctness/cleanup/coverage/consistency/DRY) over `d45bb26..HEAD` found defects in the cook output; all self-decidable ones fixed TDD (failing test first) before release.
- **CRITICAL · migrate_backfill_ids.py** — `--apply` rewrote `approved` artifacts under blanket `--confirmed-by`/`--date`; the approved-protection test was a phantom (dry-run only). Fix: approved artifacts skipped from writes unless per-id `--confirm-approved <ID>`; real apply-path test asserts byte-unchanged without it, written with it. See DEC-7. (GATE-NO-SILENT-REVERSAL.)
- **CRITICAL · render_html.py** — `staleness_banner()` was computed + unit-tested but never injected into the rendered page (F04 coverage was phantom). Fix: `write()` now injects the banner into the page body on drift>0; integration test asserts the banner text appears in the written HTML and is absent when in sync. F04 now genuinely delivered.
- **HIGH · change_log_writer.py** — dedup checked date/artifact/action as independent whole-file substrings → dropped genuinely-distinct triples (audit-trail data loss). Fix: parse existing month file into `(date,artifact,action)` tuples and dedup by tuple; contamination test added.
- **HIGH · analyze_telemetry.py / harvester.py** — export-summary default path hand-derived to `.claude/skills/telemetry/` (wrong) and ignored `CK_TELEMETRY_DIR`. Fix: use `telemetry_paths.TELEMETRY` (single home); env-honoring test added.
- **HIGH · snapshot.py restore** — data-loss window (partial-swap failure orphaned the live tree) + crash on absent spec_root (recover-deleted-tree). Fix: skip backup-rename when spec_root absent; roll backup back to spec_root on swap failure; re-added the dropped unknown-ts negative test.
- **MED · migrate_backfill_ids.py** — stopped stamping `schema_version` (orthogonal to id; risked era-2 downgrade); normalized `skipped` paths to repo-relative; re-added BOM/comment round-trip test (DEC-7).
- **MED · check_consistency_product.py** — persona-portrait false-positive on descriptive headings (`## Alice — the busy admin` vs persona `Alice`). Fix: heading matches on persona-name lead token + separator; negative test added. Dropped unused `root` param to match sibling `(graph)` signature.
- **LOW · track_artifact_edits.py** — `_is_spec_artifact` over-captured `docs/productx/` (no path boundary). Fix: require segment boundary. **LOW · decision_register_view.py** — removed dead `_visited` param.
- **Verify:** product-spec 779 passed / 1 pre-existing env-only dogfood failure; telemetry+hooks+_shared 241 passed. Append-only integrity of shared telemetry files (P6↔P7) re-confirmed INTACT by review.
