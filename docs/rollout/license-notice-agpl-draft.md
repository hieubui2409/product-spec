...
2.3.0 đưa `LICENSE` (và mã nguồn) vào cây cài của bạn để bản bạn đang giữ tuân thủ đầy đủ.

*(Chủ kit: xác nhận cách diễn đạt "hiệu lực với bản trước 2.2.1" cho đúng ý định cấp phép.)*

## 2. Đoạn release-notes (để chủ kit dán vào `[Unreleased]` của `CHANGELOG.md` lúc tag 2.3.0)

> **Lưu ý**: KHÔNG tự sửa heading version trong `CHANGELOG.md` ở phiên này. Lúc phát hành,
> chủ kit dán đoạn dưới vào mục `## [Unreleased]` rồi chạy `release.py --release 2.3.0 --apply`
> (lệnh này tự khoá `[Unreleased]` → `[2.3.0]` + bump `pack.manifest.yaml`).
...
- The bundle is licensed under AGPL-3.0; from 2.2.1 onward every distributed bundle
  ...
  2.3.0 upgrade pack adds `LICENSE` (and source) into the installed tree so an existing
...
- [ ] Đính mục 1 kèm [thông điệp mời PO](po-invitation-2.3.0.md).
- [ ] Dán mục 2 vào `CHANGELOG.md` `[Unreleased]` khi phát hành 2.3.0.
