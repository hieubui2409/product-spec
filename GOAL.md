...
| 11 | ✅ DONE | **37** (0 CRIT · 1 HIGH · ~11 MED · ~25 LOW) + 1 extra crash site (fix-verify) → all fixed; 2 accepted (owner), 1 refuted. Watch = C10 regressions. 59 agents / ~3.8M tok |
| 12 → 15 | ⏸️ PENDING | Full C7-style run mode (below). Stop after 2 consecutive zero-finding cycles; cap C15 |

**Convergence:** NOT converged (counter = 0; C11 had 37 findings). BUT clear downward trend: C9 101 → C10 67 → **C11 37**; CRIT count 3 → 0 → 0 (C9 CRITICALs still hold). C11's single HIGH (`check_traceability` unhashable brd_goals **element** → set-membership TypeError) is the SAME crash-family the C9/C10 unhashable sweeps closed at sibling sites — a missed third site, not a new mechanism. The PRIMARY-WATCH on C10 regressions earned its keep: the top finding was a C10 stale-caller (`spec_graph._competitors` did not get the C10 malformed-`type:` coercion hoist that `build_nodes` got → competitors silently vanished on a hand-edited `type: [brd]`). Remainder = doc-drift + DRY/dead-code + adversarial-input hardening. Convergence plausibly near if C12 keeps trending; cap C15.

**Test state (green):** `product-spec` **398** (was 382; +8 C11 + intervening test adds) · `claude-pack` **126** (was 117; +9 C11 regression tests). (as of C11 close, 2026-05-31). acme-shop strict_gate 0/0; 42/42 viz combos (14 views × 3 formats) OK.
...
- **C11:** `spec_graph._node_type(art)` single-source type resolution (build_nodes + `_competitors` + `_product_meta` now share it — malformed `type:` coerces identically, competitors no longer vanish); `check_traceability` brd_goals per-element `isinstance(str)` guard (unhashable element no longer crashes the gate/strict_gate); `render_html.product_name` coerces non-str name + non-dict product → str at the single source (fixes export/board/explorer chrome + md heading); `render_html.goal_detail_md` metrics `isinstance(list)` shape-guard (no char-split); `render_html._competition_matrix` cell_lookup now REQUIRED (dropped the dead inline-resolve fork + `_PARITY_COLS`); `render_ascii._product(graph)` dict-guard for `_ascii_product_name`/`persona`; shared `render_ascii._scalar` hoist (render_board + render_explorer import it); `generate_templates.render` strips leading comment BEFORE residual-token check; `migrate` threads glob `art_type` into `apply_file` + `_type_for_path` brd.md scoped to `parent=="product"`; claude-pack `build_manifest._assemble_manifest` passes malformed scalar category through (no `list()` char-split/TypeError) → validate E010; `manifest_loader._as_list` guard on the 4 slug-resolution loops + `_check_path_safety` non-list no-op + `_include_shared` E010 type-guard (covers both validate read-sites + gates cli.py/selection); `args.resolve_epoch` `_MAX_EPOCH` ceiling (year-9999, both flag + env); `templates.render_template` catches `UnicodeDecodeError`; `selection._walk_dir` `arc_prefix.strip('/')` (trailing-slash extra no double-slash arcname); dead `selection.claude_dir_resolved` + `render_ascii` time-view `nodes_by_id` + `render_mermaid` vestigial f-string removed. Many doc-drift fixes (view counts 9→11/12, --lang scope to prose/visual scripts, anchors/visualize exit-code clarity, anchor `file` key, delta no edge-diff, E101 message, E071/E072 search-root-absent variant, --bundle-name CLI default, pin-within-category advice). **Watch:** the `_node_type` hoist (any 4th type-resolution site?); the `_competition_matrix` required-arg change (any other caller?); the residual-token reorder (any template relying on pre-strip detection?).
...
**Cycle 11 (37 → all fixed + 1 extra; 2 accepted, 1 refuted):** FULL C7-style via Workflow (10 subsystem + 4 cross-cutting angle + 2 reference finders, all 9 angles, dedup → adversarial 3-state verify → 2 sweep). 59 agents / ~3.8M tok. PRIMARY WATCH = C10 regressions. **0 CRIT · 1 HIGH** (`check_traceability` unhashable brd_goals **element** → set-membership TypeError crashes gate+strict_gate; the C9/C10 unhashable sweeps closed sibling sites at `check_consistency._status_inconsistency`/`spec_graph` but missed this one → per-element `isinstance(str)` guard). **Top MED = the C10 stale-caller** the watch was for: `spec_graph._competitors` computed `ntype = fm.get("type") or hint` — a truthy non-string `type: [brd]` short-circuited the `or`, so competitors silently vanished while `build_nodes` (which GOT the C10 coercion) still materialized the goals on the SAME file → single-source `_node_type(art)` used at all 3 type-resolution sites. Other fixes: `render_html.product_name` non-str/non-dict coerce (1 source for chrome+heading), `goal_detail_md` metrics shape-guard, `render_ascii._product` dict-guard, shared `_scalar` hoist, dead `_PARITY_COLS`/`nodes_by_id`/`claude_dir_resolved`/vestigial-f-string removal, `_competition_matrix` cell_lookup required (dropped dead fork), `generate_templates` comment-strip-before-residual-check, `migrate` single type resolution; claude-pack `_assemble_manifest` scalar passthrough→E010, `manifest_loader._as_list` on 4 loops + `_check_path_safety` non-list no-op + `_include_shared` E010 guard, `resolve_epoch` year-9999 ceiling, `render_template` UnicodeDecodeError, `selection` trailing-slash arcname. **+1 EXTRA crash site in fix-verify** (the int-category regression test exposed `_check_path_safety`/slug-loops iterating a non-list category → `_as_list` + guard). **Owner interview (3):** Q1 `--lang`/dashboard-chrome doc-drift → **narrow SKILL.md** (graph/HTML-native page chrome stays English; no code change); Q2 board column-vs-facet key divergence → **accept as documented** (cosmetic, only on validator-flagged malformed enum); Q3 ASCII persona int/str collision → **accept as-is** (C10 str() key correct; personas documented string labels). **REFUTED 1** (`check_consistency.check_depends_on` out-of-band AC enrich = non-issue). +8 product-spec / +9 claude-pack regression tests (390→398, 117→126); acme-shop 0/0; 42/42 viz combos. Lead: full implementation (surgical edits, no sub-batch needed at this finding volume).
