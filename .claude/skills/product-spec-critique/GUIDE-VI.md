...
## 1. product-spec-critique là gì, và khác `--validate` ở chỗ nào
...
Đây là skill thứ hai trong một cặp: **`product-spec` dựng spec; `product-spec-critique` xé nó ra** (và không bao giờ
sửa nó). Spec phải tồn tại trước đã.

---

## 2. Khái niệm cốt lõi — mô hình tư duy (đọc một lần)

1. **Bốn lăng kính → một báo cáo, không bao giờ sửa.** Bốn nhà phê bình product / tech / market / craft chạy (song
   song), một bước hợp nhất gộp lại + một bước "người hóa" mài giũa, kết quả là **một báo cáo markdown** trong
   `docs/product/critique/`. Skill **không bao giờ động vào spec** — bạn tự đưa phát hiện về (khái niệm 6).
2. **Quan điểm, không đạt/trượt → không bao giờ CI.** Vì nó có quan điểm + châm biếm + tra mạng nên **bản chất không tất
   định**, cố tình để ngoài cổng CI. `--validate` là cổng tất định; cái này là phán xét chồng lên trên.
3. **Soi theo chuỗi tổ tiên.** Kể cả một story đơn lẻ cũng được chê dựa trên **toàn bộ chuỗi tổ tiên** (epic → PRD →
   mục tiêu → tầm nhìn) — một story chỉ có nghĩa khi soi cùng ý đồ ở các tầng trên.
4. **Bậc thang giọng, mức 1–9.** Mức 5 (`--no-mercy`) là **mức mặc định** và **không bị chặn cổng**. Mức 1–4 không công
   kích người viết; mức 5 gỡ lằn ranh đó; **mức 6–9 nguy hiểm** (chửi thẳng / năng lực / tính cách / chửi thề) và không
   bao giờ dùng nơi chuyên nghiệp. **Mức 9 hỏi lại mỗi lần chạy.** Lằn ranh đầy đủ ở dưới.
5. **Lằn ranh không-bao-giờ-vượt giữ ở mọi mức — ĐỐI TƯỢNG quyết định, không phải độ gắt.** Nó chửi vào *công việc*;
   không bao giờ nhắm vào *con người bạn*. Lằn ranh đầy đủ ở dưới.
6. **Nó nối về spec bằng quyết định, không sửa.** Một phát hiện đáng-ghi-quyết-định chỉ thành `DEC-<n>` **khi bạn xác
   nhận**, và việc đưa cả vòng về spec do `product-spec --apply-critique` làm — skill này chỉ ghi báo cáo.
7. **Chạy lại tái sử dụng việc cũ (rẻ hơn, không yếu đi).** Từ lần chê thứ hai của một phạm vi, phần phân tích đã lưu
   được dùng lại; lỗi chặn của cha có thể **kế thừa** xuống và phán quyết của con **tổng hợp** lên cha.
...
- **Lần đầu:** trỏ vào toàn bộ spec ở mức mặc định — `/product-spec-critique` — và **đọc báo cáo** (bảng đếm mức độ →
  Top-3 sửa ngay → theo lăng kính). Một lần chạy đó cho bạn thấy hình hài đầu ra.
- **Rồi thu hẹp:** soi một nhánh (một PRD/epic/story), chọn một hai lăng kính, đặt `critique_detail_level`, thử
  `--no-web` khi muốn chê chỉ dựa trên spec. Lặp trên một nhánh rẻ hơn nhiều so với chạy lại `all`.
- **Giọng, cẩn thận:** giữ mức ≤ 5 cho bất cứ thứ gì sẽ chia sẻ. Mức 6–9 là chế độ "tự hành xác" riêng tư chỉ dành cho
  chính tác giả của spec — hiểu lằn ranh + sàn-an-toàn trước khi với tới chúng.
- **Khi tích hợp:** dùng các hành vi tái sử dụng/kế thừa/tổng hợp, việc bắt lỗi lặp lại qua các báo cáo, và **cầu nối
  DEC** để một bản chê biến thành quyết định được ghi lại trong `product-spec` (`--apply-critique`).

---

## 4. Những lưu ý quan trọng

- **Không bao giờ sửa spec.** Chỉ ghi báo cáo. Phát hiện chỉ về spec qua `product-spec --apply-critique`, nơi bạn xác
  nhận từng cái. (§2.1, §2.6)
- **Không bao giờ làm cổng CI.** Quan điểm + web + giọng = không tất định, cố tình để ngoài cổng tái lập. (§2.2)
- **Phạm vi `all` tốn kém** (opus × 2 + web). Soi theo nhánh khi đang lặp; để dành `all` cho một lượt rà toàn bộ.
- **Mức 5 là mặc định không bị chặn** — có thể đá xéo cả bạn mà không cảnh báo. **6–8 cảnh báo + xác nhận** khi gọi
  ad-hoc; **9 hỏi lại mỗi lần chạy** và tụt xuống 8 nếu từ chối. Không mức nào trong 6–9 dùng cho môi trường chuyên nghiệp.
- **Lằn ranh không-bao-giờ-vượt luôn giữ**, kể cả mức 9, kể cả khi bạn đồng ý — công việc thì được, con người bạn thì
  không. (§2.5)
- **Spec phải tồn tại trước** — chạy `cleanmatic:product-spec` trước đã. Skill này không viết, không validate, không
  tách nhỏ, không vẽ sơ đồ.
...
## Tier B — Thu hẹp & tinh chỉnh

### B1. Chê một nhánh: một PRD, một epic, hay một story
...
### B4. Hai mức "chi tiết" khác nhau (spec vs critique)
...
### B5. Chạy ngoại tuyến, không tra mạng
...
### C2. Cấu hình giọng cho mức 7-9 (giới, phương ngữ, chửi thề)
...
### C3. Cảm giác của từng mức giọng (cùng một lỗi)
...
## Tier D — Tái sử dụng & tích hợp

### D1. Chê lại lần hai: tái sử dụng & ngữ cảnh cha-con
...
### D2. Bắt lỗi lặp lại và đọc lại báo cáo cũ
...
### D3. Cầu nối sang Quyết định (DEC)
...
> lý do để sau này phân biệt với quyết định sinh ra từ `--validate`. Nó không bao giờ tự ý sửa một hạng mục đã duyệt. Để
> đưa cả một báo cáo về spec, dùng `product-spec --apply-critique <báo-cáo>`.

---

## 6. Bốn lăng kính chê những gì
