...
| `audit` | `--viz audit [--format ascii\|md\|html] [--lang en\|vi]` | **governance trail** (C9): chronological join of change-log · approval metadata · stale-approvals · decision register → *when · artifact · action · who-approved · what-drifted · DEC*. Reuses `status.py` loaders (DRY). Source disagreement (an approval/stale flag with no change-log/DEC corroboration) renders an explicit **`unreconciled`** row — never dropped (H7). Formats: **`ascii` (default) · `md` · `html`**; the HTML form escapes EVERY dynamic field server-side via `render_html._escape` (no DOMPurify/marked, no href channel) — the XSS-watch guard is a `bug_class` test (`test_html_escapes_malicious_dynamic_fields`). No Mermaid form (a timeline table is not a graph → `--format mermaid` falls back to ascii). Assembler: `assemble_audit_trail.py`; HTML fragment: `render_html.audit`. |
