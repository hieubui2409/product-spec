...
Adds the COMPETITION dimension to the spec model:
  ...
  - OpSec: the parser IGNORES any competitor `url` beginning with `private:`
    — it must never reach the graph or any render.
  - Views: an HTML-native parity matrix (competitors rows × PRDs cols, cells =
    parity enum) + threat heatmap; NOT Mermaid.
...
ids 11-13), not here. Its deterministic SCRIPT half — the parity-resolution
...
# ---------------------------------------------------------------------------
# Competitors at the BRD parse into the graph (DRY identity home).
# ---------------------------------------------------------------------------

def test_brd_template_carries_competitors_key():
    """The freshly generated brd.md template must carry a `competitors:`
    frontmatter key — the DRY home for competitor identity. RED today:
    ...
    assert "competitors:" in front, (
        "brd.md template must carry a `competitors:` frontmatter key "
...
# ---------------------------------------------------------------------------
# PRD references competitors by ID via the competitive_parity MAP.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# OpSec: a `private:`-prefixed competitor URL is IGNORED by the parser
...
def test_private_url_skipped(tmp_path):
    ...
    OpSec chokepoint): the secret path must not survive onto
...
def test_private_url_not_in_render(tmp_path):
    ...
    threat-heatmap HTML (the URL is encouraged-public only, never
...
# ---------------------------------------------------------------------------
# Parity matrix + threat heatmap render HTML-native (NOT Mermaid).
# ---------------------------------------------------------------------------
...
def test_parity_matrix_render(tmp_path):
    ...
    value for that (competitor, PRD) pair."""
    proj = _scaffold(tmp_path)
    _write_brd(proj, _BRD_TWO_COMP)
    _write_prd(proj, """---
id: PRD-AUTH
...
""", name="checkout.md")
    ...
    # Columns = PRD ids (feature-area = PRD).
...
def test_threat_heatmap_render(tmp_path):
    """The threat heatmap surfaces each competitor × threat level. Render
...
def test_competition_render_escapes_spec_text(tmp_path):
    """Injection guard (chokepoint discipline): a competitor name with
...
def test_competition_render_deterministic(tmp_path):
    """Same graph → byte-identical competition fragment (no timestamp
...
# ---------------------------------------------------------------------------
# Back-compat. A v1 spec with NO competitors / NO competitive_parity
...
def test_competition_backcompat(tmp_path):
    ...
    empty competitor set to the graph (competition fields are optional)."""
...
def test_competition_render_backcompat_empty(tmp_path):
    ...
    renders an empty/placeholder matrix. Guards the renderer's empty path
