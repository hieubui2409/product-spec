...
`release` gathers a **curated subset** of `.claude/` (plus a few optional top-level files like
...
> 💡 Not sure what to do? Just type `/cleanmatic:release` (no flag): if no manifest exists, the skill runs
...
> - *Bundle version* (SemVer)? Default `0.1.0`. *Bundle name*? Default `product-spec`.
> - **Available skills:** product-spec, release. You pick `product-spec`.
...
```
/cleanmatic:release            # no flag → interactive interview flow
```
...
> then write `dist/product-spec-1.0.0.tar.gz` plus the `.sha256` sidecar. The build is deterministic — the
...
> `dist/product-spec-0.2.0-rc1.tar.gz`.
...
> underlying command above directly. This is exactly what the release workflow (`release.yml`) does.
...
**When to use:** The recipient has `product-spec-{version}.tar.gz`.
...
> ℹ️ This step is **outside** the `release` skill — it's a recipient-side operation run by hand, not via a
