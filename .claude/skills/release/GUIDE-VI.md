# Hướng dẫn sử dụng `release` cho Developer
...
`release` gom một **tập con được chọn lọc** của `.claude/` (cùng vài file top-level tùy chọn như
...
```
/cleanmatic:release --manifest .claude/pack.manifest.yaml --dry-run
```
...
> 💡 Không chắc làm gì? Cứ gõ `/cleanmatic:release` (không kèm flag): nếu chưa có manifest, skill chạy
...
> - *Version* bundle (SemVer)? Mặc định `0.1.0`. *Bundle name*? Mặc định `product-spec`.
> - **Skills khả dụng:** product-spec, release. Bạn chọn `product-spec`.
...
```
/cleanmatic:release            # không flag → luồng phỏng vấn tương tác
```
...
> ghi `dist/product-spec-1.0.0.tar.gz` cùng `.sha256` sidecar. Build deterministic — cùng input sẽ ra
> byte-identical.

#### Cờ lệnh skill tương đương

```
/cleanmatic:release --manifest .claude/pack.manifest.yaml
```
...
```
/cleanmatic:release --manifest .claude/pack.manifest.yaml --dry-run
/cleanmatic:release --manifest .claude/pack.manifest.yaml --dry-run --compute-sha
```
...
>
> **Skill:** Override `version=0.2.0-rc1` cho lần này, manifest giữ nguyên. Output: `dist/product-spec-0.2.0-rc1.tar.gz`.

#### Cờ lệnh skill tương đương

```
/cleanmatic:release --manifest .claude/pack.manifest.yaml --version 0.2.0-rc1
/cleanmatic:release --manifest .claude/pack.manifest.yaml --bundle-name my-pack
```
...
```
/cleanmatic:release --manifest .claude/pack.manifest.yaml --skills product-spec --extra docs/onboarding.md
/cleanmatic:release --manifest .claude/pack.manifest.yaml --agents planner,researcher --rules primary-workflow.md,development-rules.md
```
...
```
/cleanmatic:release --manifest .claude/pack.manifest.yaml --include-readme --include-claudemd --include-settings
/cleanmatic:release --manifest .claude/pack.manifest.yaml --include-scripts --include-schemas
```
...
```
/cleanmatic:release --manifest .claude/pack.manifest.yaml --include-shared lib,utils
/cleanmatic:release --manifest .claude/pack.manifest.yaml --follow-shared
/cleanmatic:release --manifest .claude/pack.manifest.yaml --strict
```
...
```
/cleanmatic:release --manifest .claude/pack.manifest.yaml --source-date-epoch env
```
...
> tiếp lệnh thực thi ở trên. Đây cũng đúng là cách release workflow (`release.yml`) làm.
...
```
/cleanmatic:release --manifest .claude/pack.manifest.yaml --json
```
...
**Khi nào dùng:** Người nhận đã có `product-spec-{version}.tar.gz`.
...
> ℹ️ Bước này nằm **ngoài** skill `release` — đây là thao tác phía người nhận, chạy bằng tay, không qua
...
> ```powershell
> # PowerShell
> (Get-FileHash product-spec-1.0.0.tar.gz -Algorithm SHA256).Hash
> tar -xzf product-spec-1.0.0.tar.gz
> cd product-spec-1.0.0
...
```bash
sha256sum -c product-spec-1.0.0.tar.gz.sha256
tar -xzf product-spec-1.0.0.tar.gz
cd product-spec-1.0.0 && ./install.sh
...
```
dist/
├── product-spec-{version}.tar.gz          # bundle
