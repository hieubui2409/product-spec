...
| `--bundle-name <name>` | string | `(manifest)` | `bundle_name` | Output filename stem. Must match `^[a-zA-Z0-9][a-zA-Z0-9._-]*$`. CLI default is unset (falls back to manifest `bundle_name`; the `product-spec` literal applies only when neither CLI nor manifest sets it). |
