"""test_bundle_ships_product_spec_critique_artifacts — the live bundle ships the
product-spec-critique skill, its 5 lens/consolidate agents, and the drift-nudge hook.

The 5 `product-spec-critique-*` agents and `product_spec_critique_nudge.py` live at the repo's
...
the `skills: [product-spec-critique]` slug walk. These tests guard that contract against
...
HOOK_FILE = "product_spec_critique_nudge.py"
SKILL_SLUG = "product-spec-critique"

AGENT_ARCS = [f".claude/agents/{a}" for a in AGENT_FILES]
HOOK_ARC = f".claude/hooks/{HOOK_FILE}"
SKILL_PROBE_ARC = ".claude/skills/product-spec-critique/SKILL.md"
...
def test_manifest_lists_product_spec_critique_skill_agents_hook():
    manifest = _load_real_manifest()
    assert SKILL_SLUG in manifest.get("skills", []), \
        "skills: must list product-spec-critique"
...
def test_selection_bundles_product_spec_critique_artifacts():
    manifest = _load_real_manifest()
...
def test_critique_scan_script_bundled():
    ...
    manifest = _load_real_manifest()
    arcs = {arc for _, arc in resolve_selection(manifest, REPO_ROOT)}
    assert ".claude/skills/product-spec-critique/scripts/critique_scan.py" in arcs
